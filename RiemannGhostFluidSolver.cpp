//Riemann Ghost Fluid method implemented with HLLC solver for the Euler equations
//incorporates stiffened gas EOS
...
Prim::Prim(double _rho, double _u, double _p, double _gamma, double _p_inf){
    rho = _rho; u = _u; p = _p; gamma=_gamma; p_inf=_p_inf;    
}

class ConsU{
    public:
        ConsU(double, double, double, double, double);
        double rho;
        double rhou;
        double E;
        double gamma;
...
ConsU::ConsU(double _rho, double _rhou, double _E, double _gamma, double _p_inf){
    rho = _rho; rhou = _rhou; E = _E; gamma = _gamma; p_inf=_p_inf;
}
...
ConsU primtoconsU(Prim W){
    ...
    double E = 0.5*W.rho*pow(W.u,2) + (W.p+W.gamma*W.p_inf)/(W.gamma-1);
    double gamma = W.gamma;
    double p_inf = W.p_inf;
    ConsU U(rho, rhou, E, gamma, p_inf);
    return U;
}
...
Prim consUtoprim(ConsU U){
    double rho = U.rho;
    double u = U.rhou/rho;
    double p = (U.gamma-1)*(U.E - 0.5*rho*pow(u,2)) - U.gamma*U.p_inf;
    double gamma = U.gamma;
    double p_inf = U.p_inf;
    Prim W(rho, u, p, gamma, p_inf);
    return W;
}
...
ConsU delta(double omega, ConsU U_l, ConsU U_m, ConsU U_n, string scheme){
     ...
     */
    ConsU delta_i_l(U_m.rho - U_l.rho, U_m.rhou - U_l.rhou, U_m.E - U_l.E, 0, 0);
    ConsU delta_i_n(U_n.rho - U_m.rho, U_n.rhou - U_m.rhou, U_n.E - U_m.E, 0, 0);
    
    //alternative conventions for (special case) dividing by zero RHS slope:
    ...
    ConsU delta_i(0.5*(1+omega)*delta_i_l.rho+0.5*(1-omega)*delta_i_n.rho,
                  ...
                  0.5*(1+omega)*delta_i_l.E+0.5*(1-omega)*delta_i_n.E, 0, 0);

    ConsU sigma_R(2/(1-omega+(1+omega)*r_rho), 2/(1-omega+(1+omega)*r_rhou),
                  2/(1-omega+(1+omega)*r_E), 0, 0);
...
class Cell{
    public:
        ...
        Cell(Prim WL, Prim WR):
            ...
            UL(0,0,0,0,0), UR(0,0,0,0,0), ULstar(0,0,0,0,0), URstar(0,0,0,0,0),
            FL(0,0,0), FR(0,0,0), FLstar(0,0,0), FRstar(0,0,0),
            U(0,0,0,0,0), Fout(0,0,0) {
            //direct wave speed estimates:
            aL = pow(fabs(WL.gamma*(WL.p + WL.p_inf)/WL.rho), 0.5);
            aR = pow(fabs(WR.gamma*(WR.p + WR.p_inf)/WR.rho), 0.5);
            SL = ((WL.u - aL < WR.u - aR)? WL.u - aL : WR.u - aR);
            SR = ((WL.u + aL > WR.u + aR)? WL.u + aL : WR.u + aR);
            Sstar = ((WR.p) - (WL.p)\
                    + WL.rho*WL.u*(SL-WL.u) - WR.rho*WR.u*(SR-WR.u))/\
...
vector<Prim> updateW(vector<Prim> W_vec, int n, double &dt, double dx, double CFL, double &Smax,
    ...
    ConsU Un(0,0,0,0,0);
    ConsU UR(0,0,0,0,0);
    ConsU UbarL(0,0,0,0,0);
    ConsU UbarR(0,0,0,0,0);
    Prim WbarL(0,0,0,0,0);
    Prim WbarR(0,0,0,0,0);
    ConsF FbarL(0,0,0);
    ConsF FbarR(0,0,0);    
    double omega = 0;
    ConsU delta_i(0,0,0,0,0);
...
Vector update_level_set(Vector level_set, vector<Prim> W1_vec, vector<Prim> W2_vec, 
    ...
    double eps_x; //sub-cell distance in linear interpolation 
    for(int i = 0; i<n; ++i){
        if(i == 0 || i == n-1){
            if(level_set[i] <= 0){
...
void RiemannGhost(vector<Prim> &W1_vec, vector<Prim> &W2_vec, int index_LG, int n, int nG,
        bool set_all_G){
    //sets left and right side ghost fluids to Riemann star states
    //stealthy update by reference using pointers (&) very ghost like :0  
    ...
    Cell cell_LG(W1_vec[index_LG], W2_vec[index_LG+1]);
    Prim W_Lstar = consUtoprim(cell_LG.ULstar);
    Prim W_Rstar = consUtoprim(cell_LG.URstar); 
...
vector< vector<Prim> > generate_solution(TestCase CASE, Vector time_vec, int nG, bool set_all_G,
                                            bool MUSCL, string scheme, bool re_init){
    int n = CASE.n;
    double CFL = CASE.CFL;
    double T0 = CASE.T0;
    double Tf = CASE.Tf;
    ...
    //need to figure out how to keep time consistent when calc is within update function
    
    while(T < Tf){
        RiemannGhost(W1_vec, W2_vec, index_LG, n, nG, set_all_G);
        if(num_states >= 3){
            RiemannGhost(W2_vec, W3_vec, index_LG2, n, nG, set_all_G); 
        }
        if(num_states >= 4){
            RiemannGhost(W3_vec, W4_vec, index_LG3, n, nG, set_all_G); 
        }     
        //RiemannGhost fixes the ghost cell properties
        Smax = 0;
...
void tellmethings(TestCase CASE, int nG, bool set_all_G, bool re_init, bool MUSCL){
    cout << "       ###, ,##, ,##,\n        #  # #  # #  #\n        ###  #  # #  #\n        #  # #  # #  #\n        ###' '##' '##'\n             .--,\n           /  (\n           /    \\ \n          /      \\ \n         /  0  0  \\ \n ((()   |    ()    |   ()))\n \\  ()  (  .____.  )  ()  /\n  |` \\_/ \\  `""`  / \\_/ `|\n  |       `.'--'.`       |\n   \\        `""`        /\n    \\                  /\n     `.              .'    ,\n      |`             |  _.'|\n      |              `-'  /\n      \\                 .'\n       `.____________.-' " << endl;
    cout << "|| " << CASE.title << " || " << endl;
    cout << "Riemann Based Ghost fluid solver" << endl; 
...
int main(){
    ...
    TestCase CASE("AirWater");          //test case as defined in TestCases_GhostFluid.H
    ...
    Vector time_vec;
    vector< vector<Prim> > MATRIX = generate_solution(CASE, time_vec, nG, set_all_G,
                                                        MUSCL, scheme, re_init);
    CASE.nT = MATRIX.size();
    
    //print simulation details to terminal:
    tellmethings(CASE, nG, set_all_G, re_init, MUSCL); 
